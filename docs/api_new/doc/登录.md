...
登录流程：先调用旧版 API 获取 `accessToken`、`accessSecret` 和 `cookie`（参见 `docs/api_old/API.md#user/login`）。再携带这些 `cookie` 与头 `X-Uestc-Bbs: 1` 调用 `/_/auth/adoptLegacyAuth` 获取 `Authorization`，并把 `Set-Cookie` 返回的 `newbbs_auth` 合并到现有 `cookie` 中。后续请求需同时携带 `Authorization` 与合并后的 `cookie`。

`cookie`和`Authorization`都有有效期限制，具体时间暂不知晓。如要长期运行请配置自动定时重新登录。

## 获取 accessToken、accessSecret 和 cookie（旧版 API）

旧版登录使用 `user/login`，详见 `docs/api_old/API.md#user/login`。接口返回的 `token`/`secret` 分别对应 `accessToken`/`accessSecret`，同时从响应的 `Set-Cookie` 中获取 `cookie`。

## 获取Authorization

此接口需要旧版登录获取的`cookie`，并将`Set-Cookie`返回的`newbbs_auth`合并到现有`cookie`中。
